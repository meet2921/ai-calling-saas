...
#                 lead_status = bolna_to_lead_status.get(status_value, "calling")
#                 if lead_status == "failed" and status_value in ("rejected", "no-answer", "busy", "canceled"):
#                     if lead.retry_count < lead.max_retries:
#                         lead.retry_count += 1
#                         lead.status = LeadStatus.PENDING  # re-queue
#                         logger.info(
#                             f"Lead {lead_id} re-queued for retry "
#                             f"({lead.retry_count}/{lead.max_retries}) "
#                             f"reason: {status_value}"
#                         )
#                     else:
#                         lead.status = LeadStatus.FAILED
#                         logger.info(
#                             f"Lead {lead_id} permanently failed "
#                             f"after {lead.retry_count} retries"
#                         )
#                 else:
#                     lead.status = LeadStatus(lead_status)
